...
import './style.css';
declare global { interface Window {__MOONMMDB_DATA__: {worker: string; samples: Record<string,string>; licenses: string; core_sha256: string; hash_bridge_sha256: string; worker_sha256: string}} }
...
function Help({close}: {close: ()=>void}) {
  ...
  return <dialog ref={dialog} onCancel={close} aria-labelledby="help-title"><div className="dialog-head"><h2 id="help-title">使用说明</h2><button onClick={close}>关闭</button></div>
    ...
    <h3>日志分析</h3><p>分别指定 City、ASN 角色并选择一个 JSONL 日志，默认读取 /ip。统计请求次数，不去重。错误行继续处理；UTF-8 损坏、读取失败或资源超限则停止，不生成完成报告。日志默认 8 MiB、10,000 行，最多 64 MiB、1,000,000 行；单行最多 8 MiB。日志原文不包含在报告中。</p><h3>资源与取消</h3><p>最多两份库，单库及合计不得超过 256 MiB。默认检查一亿工作单位、64 MiB 辅助状态；这不是浏览器内存上限。取消会终止后台任务，需要重新加载已选文件。大文件建议使用桌面浏览器，受设备可用内存限制。</p><p>结果最多 8 MiB；预览最多 64 KiB，复制与下载保留完整结果。整数以十进制字符串保留，浮点另含原始位表示。复制受权限限制时可下载 JSON。</p>
...
function DatabaseInfo({database, index, analysis}: {database: Database; index: number; analysis?: boolean}) {
  const m = database.metadata;
  return <details className="database-info"><summary>{analysis?(index===0?'数据库 A':'数据库 B'):(index===0?'当前库／旧库':'新库')} · {database.name}</summary><dl>
...
function ResultPanel({result, notice, setNotice}: {result: Result|null; notice: string; setNotice: (s:string)=>void}) {
  ...
  return <section className="results" aria-labelledby="result-title"><div className="result-header"><h2 id="result-title">{result?.operation.op==='analyze'?'分析结果':result?.operation.op==='compare'?'对比结果':result?.operation.op==='validate'?'检查结果':'查询结果'}</h2><div className="actions"><button disabled={!result} onClick={copy}>复制 JSON</button><button disabled={!result} onClick={download}>下载 JSON</button></div></div>
    ...
    </div>{data.status==='complete'?<AnalyticsResult report={data}/>:null}{shown!.truncated ? <p className="notice">预览已截断至 64 KiB；复制和下载包含完整结果。</p> : null}<pre className="json" tabIndex={0} aria-label="结果 JSON">{shown!.text}</pre></> : <div className="empty"><span className="code-icon" aria-hidden="true">〈/〉</span><p>结果将在这里显示</p></div>}
  </section>;
}
function App() {
  ...
  function clear() {generation.current++;client.close();files.current=[];setSelectedNames([]);setDatabases([]);setStarted(null);setIp('');setFields('');setAnalysis({...initialAnalysis});resetResult();}
  ...
  async function run() {
    ...
    try {
      operation=mode==='analyze'?{op:mode,options:{file:analysis.file!,city:analysis.city,asn:analysis.asn,ipPath:analysis.ipPath,top:Number(analysis.top),maxBytes:Number(analysis.maxMiB)*1024*1024,maxRecords:Number(analysis.maxRecords)}}:mode==='validate'?{op:mode,decode,work:Number(work),state:Number(state)*1024*1024}:{op:mode,ip:ip.trim(),fields:fieldsFromText(fields)};
    } catch(e) {setError((e as Error).message);return;}
    const epoch=generation.current;resetResult();setStarted(Date.now());
    try {
      const reply=await client.request(operation,value=>{if(generation.current===epoch)setProgress(value);});
      if(generation.current!==epoch)return;
  ...
  return <><header><div className="brand">MoonMMDB <span>{VERSION}</span></div><nav aria-label="帮助链接"><button className="link" onClick={()=>setHelp(true)}>使用说明</button><span className="divider"/><a href="https://github.com/HhWw96/moonmmdb" target="_blank" rel="noreferrer">GitHub</a></nav></header>
    <div className="shell"><aside><h2>本地数据库</h2><label className={`dropzone ${busy?'disabled':''}`} onDragOver={e=>e.preventDefault()} onDrop={drop}><UploadIcon/><strong>{mode==='analyze'?'选择数据库 A':mode==='compare'?'选择旧 .mmdb 文件':'选择 .mmdb 文件'}</strong><span>或拖放文件到这里</span><input aria-label={mode==='analyze'?'选择数据库 A':mode==='compare'?'选择旧数据库':'选择数据库'} type="file" accept=".mmdb" disabled={busy} onChange={e=>{choose(e.target.files?.[0],0);e.target.value='';}}/></label>
      ...
      <form onSubmit={e=>{e.preventDefault();void run();}}><fieldset disabled={busy}>
        ...
        <button className="primary" type="submit" disabled={!databases.length || ((mode==='compare'||mode==='analyze') && databases.length!==2) || (mode==='analyze'?(!analysis.file||analysis.city===analysis.asn):(mode!=='validate'&&!ip.trim()))}>{mode==='lookup'?'查询':mode==='validate'?'开始检查':mode==='analyze'?'开始分析':'开始对比'}</button>
      </fieldset></form>{busy?<div className="running" role="status"><Elapsed start={started!}/>{mode==='analyze'&&progress?<span>已读取 {bytes(progress.bytes)} / {bytes(progress.total)}（{progress.total?Math.floor(progress.bytes*100/progress.total):100}%）· 已处理 {progress.lines} 行；读取结束后仍需完成报告</span>:null}<button onClick={cancel}>取消操作</button></div>:null}{error?<p className="error" role="alert">{error}</p>:null}</section>
