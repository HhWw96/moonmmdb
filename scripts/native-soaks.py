...
jobs=[]
try:
 for name,script,flags in [('native-soak','native-soak',[]),('inspection-soak','inspection-soak',[]),('native-diff-soak','native-soak',['--mode','diff'])]:
  log=(root/'verification/local'/f'{name}.log').open('w',encoding='utf-8')
  jobs.append((subprocess.Popen([sys.executable,'-X','utf8',str(root/'scripts'/f'{script}.py'),*flags],cwd=root,stdout=log,stderr=subprocess.STDOUT),log))
 ...
 print('All three 30-minute Native soaks passed under concurrent host load.')
finally:
