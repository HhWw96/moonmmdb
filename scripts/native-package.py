...
def verify(archive):
 ...
 for command in [['--version'],['metadata','examples/geo.mmdb'],['lookup','examples/asn.mmdb','192.0.2.1'],['project','examples/asn.mmdb','192.0.2.1','/autonomous_system_number'],['enrich-many','examples/many.json','examples/access.jsonl']]:
  ...
  if command==['--version']:assert result.stdout.strip()=='0.5.0'
 ...
 return {'status':'passed','archive':archive.name,'sha256':digest(archive),'executable_sha256':digest(exe),'without_developer_tools':True,'commands':5}
if args.verify:
 print(json.dumps(verify(args.verify.resolve())));raise SystemExit()
platform='windows-x64' if os.name=='nt' else 'linux-x64';name='moonmmdb-0.5.0-'+platform
...
(folder/'MANIFEST.json').write_text(json.dumps({'version':'0.5.0','platform':platform,'files':files,'build':report},indent=2)+'\n',encoding='utf-8')
