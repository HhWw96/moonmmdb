...
if(process.argv.includes('--publish')){
 const receipt=JSON.parse(readFileSync('verification/releases/0.7.0/registry-0.7.0.json','utf8'));
 if(receipt.status!=='passed'||receipt.version!=='0.7.0'||receipt.module!=='HhWw96/moonmmdb')throw new Error('Missing fresh Mooncakes receipt');
 const manifest=readFileSync('moon.mod','utf8');if(!/^version = "0.7.0"$/m.test(manifest))throw new Error('Manifest version mismatch');
 const paths=['dist/publish/windows/moonmmdb-0.7.0-windows-x64.zip','dist/publish/linux/moonmmdb-0.7.0-linux-x64.tar.gz'];
 const checksum=[];
 ...
 const url='https://download.mooncakes.io/user/HhWw96/moonmmdb/0.7.0.zip';
 const response=await fetch(url);if(!response.ok)throw new Error('Registry archive unavailable');
 const archive=Buffer.from(await response.arrayBuffer());
 const provenance=JSON.parse(readFileSync('verification/releases/0.7.0/publication.json','utf8'));
 ...
 const source='dist/publish/HhWw96-moonmmdb-0.7.0.zip';writeFileSync(source,archive);paths.push(source);
 checksum.push(provenance.registry_archive_sha256+'  HhWw96-moonmmdb-0.7.0.zip');
 writeFileSync('dist/publish/SHA256SUMS',checksum.join('\n')+'\n');paths.push('dist/publish/SHA256SUMS');
 if(!existsSync('docs/RELEASE_0_7.md'))throw new Error('Missing reviewed release notes');
 gh(['release','create','v0.7.0',...paths,'--repo',repo,'--target',git(['rev-parse','HEAD']),'--title','MoonMMDB v0.7.0 — Native database update comparison','--notes-file','docs/RELEASE_0_7.md']);
}
